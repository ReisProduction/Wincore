...
public static class WindowManager
{
    ...
    /// <summary>
    /// Find window handle based on WindowInfo.
    /// </summary>
    public static nint FindHandle(WindowInfo info, bool foregroundWindow = false)
    {
        ...
        if (hWnd is 0)
            ...
            else if (foregroundWindow)
                hWnd = GetForegroundWindow();
        if (hWnd is 0 && foregroundWindow) throw new ArgumentNullException($"Window= \"{title}\" or hWnd= \"{hWnd}\" is doesn't exit");
        return hWnd;
    }
    /// <inheritdoc cref="ProcessManager.FindId(ProcessInfo)"/>
    public static int FindId(ProcessInfo info) => ProcessManager.FindId(info);
    /// <inheritdoc cref="ProcessManager.FindId(string)"/>
    public static int FindId(string name) => ProcessManager.FindId(name);
    ...
    /// <summary>
    /// Toggles window and menu animations system-wide.
    /// </summary>
    public static void ToggleAnimations(bool enable)
    {
        SystemParametersInfo(SPI_SETCLIENTAREAANIMATION, 0, enable ? 1 : 0, SPIF_SENDCHANGE);
        SystemParametersInfo(SPI_SETMENUANIMATION, 0, enable ? 1 : 0, SPIF_SENDCHANGE);
    }
    /// <summary>
